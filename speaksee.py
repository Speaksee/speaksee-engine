...
parser.add_argument('--videoName',             type=str, default="sample",   help='Demo video name')
parser.add_argument('--videoFolder',           type=str, default="demo",  help='Path for inputs, tmps and outputs')
parser.add_argument('--pretrainModel',         type=str, default="weight/finetuning_TalkSet.model",   help='Path for the pretrained model')
...
def inference_video(args):
    ...
    # print(next(DET.net.parameters()).device)
...
def visualization(tracks, scores, args):
    ...
    for fidx, fname in tqdm.tqdm(enumerate(flist), total = len(flist)):
        ...
        for face in faces[fidx]:
            ...
            cv2.putText(image, f'ID:{face["track"]} | {txt:.1f}', (int(face['x']-face['s']), int(face['y']-face['s'])), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,clr,255-clr),5)
        vOut.write(image)
    vOut.release()
...
# def visualization_onlyAS(tracks, scores, args):
#     # CPU: visualize the result for video format
#     flist = glob.glob(os.path.join(args.pyframesPath, '*.jpg'))
#     flist.sort()
#     faces = [[] for i in range(len(flist))]
    
#     # 기존 방식: 각 프레임에 등장하는 모든 얼굴 정보를 faces에 저장
#     for tidx, track in enumerate(tracks):
#         score = scores[tidx]
#         for fidx, frame in enumerate(track['track']['frame'].tolist()):
#             s = score[max(fidx - 2, 0): min(fidx + 3, len(score) - 1)]  # average smoothing
...
#         if len(faces[fidx]) > 0:
#             # ✅ score가 가장 높은 face 선택
#             best_face = max(faces[fidx], key=lambda x: x['score'])
            
#             clr = (0, 255, 0)  # 초록색 (실제 화자)
#             txt = round(best_face['score'], 1)
#             cv2.rectangle(image,
#                           (int(best_face['x'] - best_face['s']), int(best_face['y'] - best_face['s'])),
#                           (int(best_face['x'] + best_face['s']), int(best_face['y'] + best_face['s'])),
#                           clr, 10)
#             cv2.putText(image, f'{txt}',
#                         (int(best_face['x'] - best_face['s']), int(best_face['y'] - best_face['s'])),
#                         cv2.FONT_HERSHEY_SIMPLEX, 1.5, clr, 5)
...
def main():
    ...
    # print(f"Default tensor device: {torch.tensor(1).device}")
